...
public:
	...
	void keyPress(EZKeySym keysym)	//Quand un touche est enfonc�
	{
		switch (keysym)
		{
	...
	void buttonRelease(int x, int y, int button) {	//Relachement du clic souris
		selected_form = nullptr;	//Quand on lache la souris on d�selectionne la forme
	}
};
...
int main(int , char * [])
{
	...
	//Lire un fichier
	try {
		if (fb.open("test.txt", std::ios::in)) {	//Ouvre le fichier en lecture
	...
	MyWindow win1(400, 320, "Projet CPP", &gestion);	//Creer un fenetre
